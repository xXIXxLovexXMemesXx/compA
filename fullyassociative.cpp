...
int Cache::getFully(int address)
{
	...
	int value; // value in block line
	if (address == cacheAddr0 || address == cacheAddr1 || address == cacheAddr2 || address == cacheAddr3 || address == cacheAddr4 || address == cacheAddr5 || address == cacheAddr6 || address == cacheAddr7)
	{
		clockX += 2;// cache access for read
		value = cblocks[blockIndx].data[wordIndx];
	}
